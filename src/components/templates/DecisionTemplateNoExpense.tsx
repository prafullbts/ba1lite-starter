...
export interface DecisionItem {
  ...
  disabled?: boolean;
  /** CalcModel range name for cumulative capability level */
  cumCapabilityRef?: string;
}

export interface DecisionTemplateNoExpenseProps {
...
export const DecisionTemplateNoExpense: React.FC<DecisionTemplateNoExpenseProps> = ({
  decisions,
  className = '',
  disabled = false,
  onChange
  ...
  const handleDecisionChange = useCallback(async (decisionId: string, value: string) => {
    ...
    try {
      ...
      await setValue(decision.refName, numericValue, 'DecisionTemplateNoExpense');
          ...
          >
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {/* Display cumulative capability level if available */}
                {decision.cumCapabilityRef && (
                  <CumulativeCapabilityDisplay cumCapabilityRef={decision.cumCapabilityRef} />
                )}
              </div>
            </CardContent>
          </Card>
...
// Helper component to display cumulative capability level
const CumulativeCapabilityDisplay: React.FC<{ cumCapabilityRef: string }> = ({ cumCapabilityRef }) => {
  const cumValue = useCalcValue(cumCapabilityRef, true);
  
  const displayValue = cumValue && !isNaN(Number(cumValue)) 
    ? Number(cumValue).toFixed(0) 
    : '-';

  return (
    <div className="mt-4 text-center text-sm text-muted-foreground">
      Cumulative Capability Level: <span className="font-semibold">{displayValue}</span>
    </div>
  );
};

export default DecisionTemplateNoExpense;